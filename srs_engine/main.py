...
from srs_engine.core.routers import auth_router, contact_router, pages_router, srs_router, websocket_router
...
def create_app() -> FastAPI:
    ...
    # Initialize trace manager with WebSocket connection

    from srs_engine.core.routers.websocket import connection_manager

    trace_manager.set_websocket_manager(connection_manager)
...
async def generate_srs(srs_data: SRSRequest):
    ...
    # Use session_id from request if available, otherwise generate one

    session_id = getattr(srs_data, 'session_id', None) or str(uuid.uuid4())
    ...
    # Import trace_manager for live tracing
    ...
    diagrams = [
        ...
        ("user_interfaces", "User Interface Diagram"),

        ("hardware_interfaces", "Hardware Interface Diagram"),

        ("software_interfaces", "Software Interface Diagram"),

        ("communication_interfaces", "Communication Interface Diagram")
    ...
    for i, (diagram_key, diagram_name) in enumerate(diagrams, 1):
        ...
        render_mermaid_png(external_interfaces_section[diagram_key]['interface_diagram']['code'], image_paths[diagram_key])

        print(f"Rendered diagram {i}/4 ")
    ...
    print("✅ All tracing phases completed successfully!")
