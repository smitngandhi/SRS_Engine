...
async def parse_uploaded_file(db: AsyncIOMotorDatabase, user_id: str, file_id: str, original_filename: str, file_type: str) -> ParseResponse:
    """
    Entry point called by the router.
    Reads the file from GridFS, runs the appropriate parse chain,
    builds UnifiedDocumentJSON, saves it to GridFS, and returns ParseResponse.
    """
    storage = FileStorage(db)
    content = await storage.get_file_by_id(file_id)
    if not content:
        raise HTTPException(status_code=404, detail="Uploaded file content not found in GridFS.")

    # We need a seekable file-like object for some parsers
    file_bytes_io = io.BytesIO(content)

    # ── Run the correct parse chain ───────────────────────
    # Note: Many parsers expect a Path, but we'll use BytesIO or temporary files if needed.
    # For simplicity, we'll write to a temporary file since pdfplumber/camelot/python-docx often prefer it.
    import tempfile
    import os
