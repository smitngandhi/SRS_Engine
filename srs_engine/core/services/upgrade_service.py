...
SESSIONS_ROOT = Path("./upgrade_sessions")
PARSED_DOCS_ROOT = Path("./parsed_docs")


def _session_path(user_id: str, file_id: str) -> Path:
    directory = SESSIONS_ROOT / user_id
    directory.mkdir(parents=True, exist_ok=True)
    return directory / f"{file_id}.json"


def _parsed_path(user_id: str, file_id: str) -> Path:
    return PARSED_DOCS_ROOT / user_id / f"{file_id}.json"


# ── Session persistence ────────────────────────────────────────────────────────

def _load_session(user_id: str, file_id: str) -> UpgradeSession:
    path = _session_path(user_id, file_id)
    if not path.exists():
        raise HTTPException(
            status_code=404,
            detail=f"Upgrade session not found for file {file_id}. Call /upgrade/srs/{{file_id}}/analyse first.",
        )
    return UpgradeSession.model_validate_json(path.read_text(encoding="utf-8"))


def _save_session(session: UpgradeSession, user_id: str) -> None:
    session.updated_at = datetime.now(timezone.utc)
    path = _session_path(user_id, session.file_id)
    path.write_text(session.model_dump_json(indent=2), encoding="utf-8")


def _load_parsed_doc(user_id: str, file_id: str) -> UnifiedDocumentJSON:
...
async def create_upgrade_session(
    ...
    """
    doc = _load_parsed_doc(user_id, file_id)
...
async def run_analysis(
    ...
    session_path = _session_path(user_id, file_id)
    if not session_path.exists():
        await create_upgrade_session(user_id=user_id, file_id=file_id)

    session = _load_session(user_id, file_id)
    doc = _load_parsed_doc(user_id, file_id)
...
async def get_upgrade_session(user_id: str, file_id: str) -> UpgradeSession:
    return _load_session(user_id, file_id)
