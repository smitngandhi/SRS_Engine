...
async def create_diagram(
    ...
    """
    1. Call LLM to generate Mermaid code
    2. Sanitize and verify syntax
    3. Save record to MongoDB
    4. Render SVG and save to disk AND GridFS
    """
    repo = DiagramRepo(db)
    fs = FileStorage(db)
    ...
    # 4. Render SVG
    try:
        disk_path = _svg_disk_path(user_id, diagram_id, 1)
        ...
        logger.error(f"diagram_service | Render/Upload failure | {e}")
...
async def regenerate_diagram(
    ...
    """
    1. Call LLM to update Mermaid code
    2. Save as a new version in MongoDB
    3. Render new SVG and save to disk AND GridFS
    """
    repo = DiagramRepo(db)
    fs = FileStorage(db)
    ...
    try:
        ...
        await fs.save_file(
            svg_bytes,
            f"{diagram_id}_v{version_number}.svg",
            {
                "type": "svg",
                "user_id": user_id,
                "diagram_id": diagram_id,
...
async def delete_diagram(db: Any, user_id: str, diagram_id: str) -> bool:
    ...
    if deleted:
        ...
        try:
            ...
            files = await fs.list_files({"type": "svg", "user_id": user_id, "diagram_id": diagram_id})
            for f in files:
                await fs.delete_file(f["metadata"])
            logger.info(f"diagram_service | Deleted all SVG versions from GridFS | id={diagram_id}")
        except Exception as e:
            logger.error(f"diagram_service | GridFS deletion failed | {e}")
