...
async def generate_srs(
    ...
    repo = JobRepo(db)
    quota = QuotaRepo(db)
    docx_limit = user.get("custom_docx_limit", 2)
    allowed = await quota.check_quota(user_id, "docx_count", limit=docx_limit)
    if not allowed:
        raise HTTPException(
            status_code=429,
            detail=f"You've reached your plan limit of {docx_limit} SRS documents.",
        )

    # 2. Persist job record — worker reads the payload from here
    ...
    # 3. Check Redis is available before we commit to the job
...
async def get_job_status(
    ...
    if job.get("status") == "pending":
        job["queue_position"] = await repo.get_queue_position(job_id)
...
def _serialize_job(job: dict) -> dict:
    ...
    Also includes queue_position for PENDING jobs.
    """
    return {
...
async def list_jobs(
    ...
    for job in jobs:
        if job.get("status") == "pending":
            job["queue_position"] = await repo.get_queue_position(job.get("job_id"))
